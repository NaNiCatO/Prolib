    ...
    return (
        <div className="ml-4 mr-4 flex flex-wrap justify-start gap-4">
            {books.length == 0 ? <h1 className="text-4xl ml-3">Looks like it's empty here...</h1> :
                books.map((book) => (
                    <Card key={book.title} className="shadow-md w-52 hover:bg-accent hover:text-accent-foreground select-none" onClick={() => redirect(`/bookDetails/${book.id}`, RedirectType.push)}>
                        <CardContent>
